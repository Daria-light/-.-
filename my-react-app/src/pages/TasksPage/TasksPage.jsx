...
function TasksPage() {
  ...
  const loadTasks = async () => {
    ...
    try {
      ...
      const soon = tasksData.filter((t) => t.status === 'Скоро дедлайн').length
      const overdue = tasksData.filter((t) => t.status === 'Просрочена').length
      const doneWeek = tasksData.filter((t) => t.status === 'Выполнена').length
        ...
        <div className={styles['tasks-toolbar']}>
          ...
          <AppSelect
            ...
            onChange={(option) => {
              ...
              setFilteredTasks(tasks.filter((t) => t.status === option.value))
            }}
          />
          <AppSelect
      ...
      <div className={styles['tasks-grid']}>
        {loading ? (
          ...
          filteredTasks.map((task) => (
            <div key={task.id} className={styles['home-module']}>
              ...
              <div className={styles['task-card-body']}>
                ...
                <div className={styles['task-meta-grid']}>
                  <div className={styles['task-meta-item']}>
                    <span className={styles['task-meta-label']}>Дедлайн:</span>
                    <span className={styles['task-meta-value']}>
                      {task.deadline
                  ...
                  <div className={styles['task-meta-item']}>
                    <span className={styles['task-meta-label']}>Тип:</span>
                    <span
                      className={`${styles['task-meta-value']} ${
                        task.type === 'urgent'
                          ? styles['task-type-urgent']
                          : styles['task-type-regular']
                      }`}
                    >
                      {task.type === 'urgent' ? ' Срочная' : ' Обычная'}
                    </span>
                  </div>
                </div>
                ...
                {task.commentary && (
                  <div className={styles['task-comment']}>
                    <span className={styles['task-comment-icon']}>💬</span>
                    <span className={styles['task-comment-text']}>
                      {task.commentary}
                    </span>
                  </div>
                )}
